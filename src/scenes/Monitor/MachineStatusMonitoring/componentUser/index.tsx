...
import { MachineDto, MachineDtoListResultDto, SearchStatusMonitorInput } from '@src/services/services_autogen';
		...
		return (
			<Card>
				...
				<Row gutter={[8, 8]} align='bottom'>
					<Col {...cssColResponsiveSpan(24, 12, 12, 8, 6, 6)}>
					...
					<Col {...cssColResponsiveSpan(24, 12, 12, 24, 6, 6)} style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "flex-end" }}>
						<Button placeholder='Tìm kiếm' type='primary' onClick={() => this.handleSubmitSearch()}><SearchOutlined />Tìm kiếm</Button>
						{(this.state.gr_ma_id != undefined || this.state.ma_id_list != undefined || this.state.ma_networkStatus != undefined) &&
				...
				<Row style={{ marginTop: 10 }}>
					...
					{this.state.visibleExportMachine &&
						<ModalExportMachineStatusMonitoringUser machineListResult={this.state.select ? this.listMachine : this.machineDtoListResult.items!} onCancel={() => this.setState({ visibleExportMachine: false })} visible={this.state.visibleExportMachine} />
					}
				</Row>
			</Card >
		)
