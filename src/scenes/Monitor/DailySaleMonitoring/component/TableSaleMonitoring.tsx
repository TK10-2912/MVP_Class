...
const TableSaleMonitoring = ( props : IProps) => {
    const initialState = {
        isLoadDone: true,
    ...
    const onAction = (item: BillingOfMachineDto, action: EventTable) => {
    ...
    const isValidLocation = async (item: BillingOfMachineDto, hasUrlMap: boolean) => {
        if (hasUrlMap) {
            dispatch({type: "SET_STATE", payload: {visibleModalGoogleMap: true, ma_name: item.ten_may,ma_mapUrl: item.ma_mapUrl, ma_gps_lat: +item.ma_gps_lat!, ma_gps_lng: +item.ma_gps_lng!}})
            // await setState({ visibleModalGoogleMap: true, ma_mapUrl: item.ma_mapUrl, ma_name: item.ten_may, ma_gps_lat: 0, ma_gps_lng: 0 })
            return;
        }
        dispatch({type: "SET_STATE", payload: {visibleModalGoogleMap: true, ma_name: item.ten_may, ma_gps_lat: +item.ma_gps_lat!, ma_gps_lng: +item.ma_gps_lng!}})
    };
    const { billingOfMachine, pagination, is_printed } = props;
    ...
    return (
        <>
                <Table
                    onRow={(record) => {
                        return {
                            ...
                            style: { cursor: props.is_printed ? '' : 'pointer' },
                        };
                    }}
                    className="centerTable"
                    rowClassName={(record) =>
                        state.key_selected === record.key ? 'bg-click' : 'bg-white'
                    }
                    scroll={props.is_printed ? { x: undefined, y: undefined } : { x: 1000, y: 600 }}
                    loading={!state.isLoadDone}
                ...
                <Modal
                    centered
                    visible={state.visibleModalGoogleMap}
                    onCancel={() => dispatch({type: "SET_STATE", payload: {visibleModalGoogleMap: false}})}
                    title={<h3>{'Vị trí máy: ' + state.ma_name}</h3>}
                    width={'70vw'}
                    footer={null}
                >
                    {(state.ma_gps_lat && state.ma_gps_lng) ?
                        <MapComponent
                            centerMap={{ lat: state.ma_gps_lat, lng: state.ma_gps_lng }}
                            zoom={15}
                            positionList={[
                                { lat: state.ma_gps_lat, lng: state.ma_gps_lng, title: 'Vị trí máy' },
                            ]}
                        />
                        :
                        <div dangerouslySetInnerHTML={{ __html: state.ma_mapUrl! }} />
                    }
                </Modal>
            </>
    )
}
export default TableSaleMonitoring