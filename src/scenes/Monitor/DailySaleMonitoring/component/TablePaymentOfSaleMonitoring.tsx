...
const TablePaymentOfSaleMonitoring = (props: IProps) => {
	const appComponentBase = new AppComponentBase({});
	const [isLoadDone, setIsLoadDone] = useState(true);
	const [key_selected, setKey_selected] = useState<string |undefined>(undefined);
	const onAction = (item: PaymentMethodDto) => {
		setKey_selected(item.key);
	}
	const { dailySaleMonitoringDto, rowSelection, start_date, end_date } = props
	...
	return (
		<>
			<Table
					...
					return {
						...
						onClick: async () => {
							if (!props.is_printed) {
								...
								let url = props.parent === "sale_detail"
				...
				rowClassName={(record, index) => (key_selected === record.key) ? "bg-click" : "bg-white"}
				loading={!isLoadDone}
				rowKey={record => "quanlymaybannuoc_index__" + JSON.stringify(record)}
				size={'middle'}
				bordered={true}
				columns={columns}
				rowSelection={props.is_printed !== undefined ? rowSelection : undefined}
...
export default TablePaymentOfSaleMonitoring;
