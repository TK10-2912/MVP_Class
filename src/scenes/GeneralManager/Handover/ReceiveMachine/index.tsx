...
export default class ReceiveMachineHandover extends AppComponentBase {
	state = {
		isLoadDone: true,
		...
		end_date: undefined,
		handover_user: undefined,
		receive_user: isGranted(AppConsts.Permission.Pages_Manager_General_Admin_Handover) ? undefined : [stores.sessionStore.getUserLogin().id!]
	}
				...
				{!this.state.visibleModalCreateUpdate &&
					<Row align='bottom' gutter={[8, 8]}>
						...
						<Col className='textAlignCenter-col-992' {...cssColResponsiveSpan(24, 12, 12, 6, 6, 6)} style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "start" }} >
							...
							{(!!this.state.ha_status || this.state.ha_status! > -1 || this.state.ha_type! > -1 || !!this.state.ha_type || !!this.state.rangeDatetime || !!this.state.handover_user || (isGranted(AppConsts.Permission.Pages_Manager_General_Admin_Handover) ? !!this.state.receive_user : "")) &&
