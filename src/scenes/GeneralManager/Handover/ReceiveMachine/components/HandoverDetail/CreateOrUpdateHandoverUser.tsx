...
import { AttachmentItem, CreateHandoverInput, CreateImportRepositoryInput, HandoverDto, ImportRepositoryDto, ProductHandoverInput, ProductImportDto, ProductInRepositoryAbtractDto, UpdateHandoverInput, UpdateImportRepositoryInput } from '@src/services/services_autogen';
...
export interface IProps {
    ...
    onCancel?: () => void;
    onSuccess?: () => void;
    onChangeUser: (us_id: number) => void;
}
const { TreeNode } = Tree;
const { Search } = Input;
    ...
    initData = async (handoverSelected: HandoverDto | undefined) => {
        ...
        if (handoverSelected !== undefined && handoverSelected.ha_id !== undefined) {
            ...
            if (handoverSelected.receive_user != undefined) {
                this.setState({ us_id_hanover: handoverSelected.receive_user })
            }
        ...
        return (
            <Card className='heightWindow' style={{ height: `${height} !important`, overflowY: "auto" }}>
                <Form
                    ...
                    >
                        <SelectUser us_id={this.state.us_id} onChangeUser={async (e) => { await this.setState({ us_id: e }); this.onChangeUser(e); await this.formRef.current!.setFieldsValue({ handover_user: e }) }} />
                    </Form.Item>
                    <Form.Item
                        ...
                        rules={[rules.required]}
                    >
                        <SelectUser us_id={this.state.us_id_hanover} onChangeUser={async (e) => { await this.formRef.current!.setFieldsValue({ receive_user: e }) }} />
                    </Form.Item>
                    <Form.Item
                    ...
                    <Form.Item label="Tệp đính kèm" name='fi_id_list' rules={[rules.required]}  >
                        <FileAttachmentsImages
                            isUpLoad={true}
                            maxLength={5}
                            files={sefl.listAttachmentItem_file}
                            isLoadFile={this.state.isLoadFile}
                            allowRemove={true}
                            isMultiple={true}
                            onSubmitUpdate={async (itemFile: AttachmentItem[]) => {
                                sefl.listAttachmentItem_file = itemFile.slice(0, 5);
