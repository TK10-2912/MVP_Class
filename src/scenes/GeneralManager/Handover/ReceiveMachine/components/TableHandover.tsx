...
export default class TableHandover extends AppComponentBase<IProps> {
	...
	render() {
		const { pagination, handoverListResult, hasAction, isPrint } = this.props
		const { expandedRowKey } = this.state;
		let action: ColumnGroupType<HandoverDto> = {
			title: "", children: [], fixed: 'right', width: 40, key: "action", render: (text: number, item: HandoverDto) => <div>
				<Button
					type='primary'
		...
		const columns: ColumnsType<HandoverDto> = [
			{ title: "STT", key: "stt_fresh_drink_index", width: 50, render: (text: string, item: HandoverDto, index: number) => <div>{pagination !== false ? pagination.pageSize! * (pagination.current! - 1) + (index + 1) : index + 1}</div> },
			{
				title: "Ảnh", key: "image", dataIndex: "pr_im_name", width: 200, render: (text: string, item: HandoverDto) =>
					<div style={{ textAlign: "center" }}>
						<Image className='no-print imageDetailProduct' 
						...
						alt='No image available' fallback="data:image/png;base64,"
						/>

					</div>

			},
			{ title: "Người bàn giao", dataIndex: "handover_user_name", key: "handover_user_name", render: (text: string, item: HandoverDto) => <div> {stores.sessionStore.getUserNameById(item.handover_user)} </div> },
			{ title: "Người nhận bàn giao", dataIndex: "receive_user_name", key: "receive_user_name", render: (text: string, item: HandoverDto) => <div> {stores.sessionStore.getUserNameById(item.receive_user)} </div> },
			{ title: "Số lượng máy bàn giao", sorter: hasAction ? (a: HandoverDto, b: HandoverDto) => (a.ma_id_list?.length || 0) - (b.ma_id_list?.length || 0) : false, dataIndex: "ma_id_list", key: "ma_id_list", render: (text: string, item: HandoverDto) => <div> {item.ma_id_list?.length} </div> },
			{
				...
				} : false, dataIndex: "productHandoverInputs", key: "productHandoverInputs", render: (text: string, item: HandoverDto) => <div> {item.productHandoverInputs?.reduce((total, product) => total + product.pr_quantity, 0)} </div>
			},
			{
				title: "Loại bàn giao",
				width: 150,
				dataIndex: "ha_status",
				key: "ha_status",
				render: (text: string, item: HandoverDto) => (
					<div>
						{hasAction ?
							<>
			...
			{
				title: "Ghi chú", dataIndex: "ha_note", ellipsis: true, key: "ha_note", render: (text: string, item: HandoverDto) => <div
					style={{ marginTop: "14px", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
					dangerouslySetInnerHTML={{ __html: item.ha_note! }}>
				</div>
			},
			{ title: "Ngày tạo", sorter: hasAction, dataIndex: "ha_created_at", key: "ha_created_at", render: (text: string, item: HandoverDto) => <div> {moment(item.ha_created_at).format("DD/MM/YYYY HH:mm:ss")} </div> },
		];
		...
		return (
			<Table
				...
				columns={columns}
				size={'small'}
				onChange={(a, b, sort: SorterResult<HandoverDto> | SorterResult<HandoverDto>[]) => {
				...
				rowClassName={(record, index) => (this.state.ha_id_selected === record.ha_id) ? "bg-click" : "bg-white"}
