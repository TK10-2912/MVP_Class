...
export default class CreateHandover extends AppComponentBase<IProps> {
    ...
    async componentDidMount() {
        ...
        if (!!handoverSelected) {
            ...
            await this.setState({ listProduct: listProduct });
        }
        this.setState({ isLoadDone: !this.state.isLoadDone })
    }
    onCancel = () => {
    ...
    onChangeSelectProduct = async (pr_id: number) => {
        ...
        let productRepository: ListProductHandOver[] = stores.sessionStore.getAllProductInRepository(this.state.us_id!, this.state.re_id)
                ...
                <Row gutter={16}>
                    <Col span={14}>
                        <Row>
                            ...
                            <Col span={16}>
                                <SelectedProductRepository re_id={this.state.re_id} us_id={this.state.us_id} productId={this.state.pr_id} onChangeProduct={async (value) => {
                    ...
                    <Col span={10} >
                        <CreateOrUpdateHandoverUser
                            onChangeRepository={this.onChangeRepository}
                            onSuccess={this.onSuccess} handoverSelected={this.props.handoverSelected} onCancel={this.onCancel} listProductImport={this.state.listProduct} />
                    </Col>
                </Row >
