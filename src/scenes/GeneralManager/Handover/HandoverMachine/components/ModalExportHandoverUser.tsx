	...
	exportToExcel = async () => {
		...
		for (const [index, item] of handoverListResult.entries()) {
			...
			const row = worksheet.addRow([
				index + 1,
				 '',
				 stores.sessionStore.getUserNameById(handover_user),
				 stores.sessionStore.getUserNameById(receive_user),
				 ma_id_list?.length,
				 productHandoverInputs?.length,
				 valueOfeHandoverStatus(ha_status),
				 ha_note,
				 moment(ha_created_at).format("DD/MM/YYYY HH:mm")]);
			row.height = 80;
