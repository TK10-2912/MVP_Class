...
export default class ImportProductRepositoryAdmin extends React.Component<IProps> {
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        if (!!item[1] && !!item[2] && !!item[3] && !!item[4] && item[5] && !!item[6] && !!item[7]) {
                            ...
                            if (product != undefined) {
                                ...
                                itemCreate.pr_tr_unit = item[2].toString();
    ...
    render() {
        const columns: ColumnsType<ProductTranferDto> = [
            ...
            { title: "Tên sản phẩm", key: "pr_tr_name", dataIndex: "pr_tr_name", render: (text: string, item: ProductTranferDto) => <div> {item.pr_tr_name} </div> },
            { title: "Đơn vị ", key: "pr_tr_unit", dataIndex: "pr_tr_unit", render: (text: number, item: ProductTranferDto) => <div> {item.pr_tr_unit} </div> },
            { title: "Số lượng", key: "pr_tr_quantity", dataIndex: "pr_tr_quantity", render: (text: string, item: ProductTranferDto) => <div> {item.pr_tr_quantity} </div> },
            { title: "Đơn giá", key: "pr_tr_unit_price", dataIndex: "pr_tr_unit_price", render: (text: number, item: ProductTranferDto) => <div> {AppConsts.formatNumber(item.pr_tr_unit_price) + " VND"} </div> },
            // { title: "Đơn vị quy đổi", key: "pr_tr_unit_quydoi", dataIndex: "pr_tr_unit_quydoi", render: (text: number, item: ProductTranferDto) => <div> {item.pr_tr_unit_quydoi} </div> },
            // { title: "Số lượng quy đổi", key: "pr_tr_quantity_quydoi", dataIndex: "pr_tr_quantity_quydoi", render: (text: string, item: ProductTranferDto) => <div> {item.pr_tr_quantity_quydoi} </div> },
            { title: "Thành tiền", key: "pr_tr_total_money", dataIndex: "pr_tr_total_money", render: (text: number, item: ProductTranferDto) => <div> {AppConsts.formatNumber(item.pr_tr_quantity * item.pr_tr_unit_price) + " VND"} </div> },
        ];
        return (
            <Modal
