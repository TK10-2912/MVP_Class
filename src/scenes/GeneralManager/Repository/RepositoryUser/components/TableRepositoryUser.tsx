...
import TableViewProductInRepository from '../../RepositoryAdmin/components/TableViewProductInRepository';
import TableExportRepository from '../../RepositoryAdmin/components/TableExportRepository';
import TableTransferRepository from '../../RepositoryAdmin/components/TableTransferRepository';
...
export default class TableRepositoryUser extends React.Component<IProps> {
	state = {
		isLoadDone: false,
	...
	render() {
		...
		const columns: ColumnsType<RepositoryDto> = [
			{ title: L('STT'), key: 'stt', width: 50, render: (text: string, item: RepositoryDto, index: number) => <div>{pagination !== false ? pagination.pageSize! * (pagination.current! - 1) + (index + 1) : index + 1}</div> },
			{
				...
				dataIndex: 're_name',
				key: 're_name',
				render: (text: string, item: RepositoryDto) => <div>{item.re_name!}</div>,
			},
			{
				title: L('Tổng số mặt hàng'),
				dataIndex: 'su_name',
				key: 'su_name',
				render: (text: string, item: RepositoryDto) => <div>{item.so_luong_mat_hang || 0}</div>,
			...
			},
			{
				...
				render: (text: string, item: RepositoryDto) => (
					<div>
						{AppConsts.formatNumber(item.repositoryDetails ? item.repositoryDetails.reduce((total, currenttotal) => total + (currenttotal.pr_total_quantity_quydoi), 0) : 0)}
					</div>
				),
			...
			},
			{
				...
				render: (text: string, item: RepositoryDto) => <div>{item.so_luong_san_pham_sap_het_hang || 0}</div>,
			...
			},
			{
				...
				render: (text: string, item: RepositoryDto) => <div>{item.so_luong_san_pham_het_hang || 0}</div>,
