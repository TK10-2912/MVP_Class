...
import DetailInfomationTranferRepositoryUser from './DetailInfomationTranferRepositoryUser';
...
export interface IProps {
	...
	isLoadDone?: boolean;
	hasAction?: boolean;
	transferRepositoryDetailResult: TranferRepositoryDto[],
	noScroll?: boolean;
	isPrint?: boolean,
	onSuccess?: () => void;
	onUpdate?: () => void;
	openBillTransferRepository?: (item: TranferRepositoryDto) => void;
...
export default class TableTranferRepositoryUser extends AppComponentBase<IProps> {
	state = {
		tr_re_id: undefined,
		expandedRowKey: [],
		keyRow: undefined,
	...
	openBillTransferRepository = (input: TranferRepositoryDto) => {
		if (!!this.props.openBillTransferRepository) {
			this.props.openBillTransferRepository(input)
		}
	}
	render() {
		const { pagination, transferRepositoryDetailResult, isPrint, actionTable } = this.props
		const columns: ColumnsType<TranferRepositoryDto> = [
			...
			{ title: "Mã cấp phát", key: "im_re_code", render: (text: string, item: TranferRepositoryDto) => <div> {item.tr_re_code} </div> },
			...
			{ title: "Đơn vị",  key: "tr_re_total_quantity", render: (text: number, item: TranferRepositoryDto) => <div>Thùng</div> },
			{
		...
		{
			title: "Chức năng", key: "im_re_note", className: "no-print center", render: (text: number, item: TranferRepositoryDto) =>
				...
				<Space>
					...
					></Button>
					{
						item.tr_re_status == eTranferRepositoryStatus.TEMPORARY.num &&
						<Button
							type="primary" icon={<ForwardOutlined />} title={'Yêu cầu nhập hàng'}
							size='small'
							onClick={(e) => { e.stopPropagation(); actionTable!(item, EventTable.ChangeStatus, eTranferRepositoryStatus.REQUEST.num) }}
						></Button>
					}
					...
					{/* {
						...
						<Button
							...
							size='small'
							onClick={() => actionTable!(item, EventTable.ChangeStatus, eTranferRepositoryStatus.CONFIRM.num)}
						></Button>
...

					{
						item.tr_re_status == eTranferRepositoryStatus.CONFIRM.num &&
						<Button
							type="primary" icon={<FileDoneOutlined />} title={'Đã nhận hàng'}
							size='small'
							onClick={(e) => { e.stopPropagation(); actionTable!(item, EventTable.ChangeStatus, eTranferRepositoryStatus.RECEIVED.num) }}
						></Button>
					}

					{
						item.tr_re_status == eTranferRepositoryStatus.RECEIVED.num &&
						<Button
							type="primary" icon={<CheckOutlined />} title={'Đã nhập hàng'}
							size='small'
							onClick={(e) => { e.stopPropagation(); actionTable!(item, EventTable.ChangeStatus, eTranferRepositoryStatus.IMPORTED.num) }}
						></Button>
					}
				</Space>
		};
		...
		return (
			<Table
								...
								<>
									...
									<Tabs defaultActiveKey={tabManager.tab_1}>
										<Tabs.TabPane tab={tabManager.tab_1} key={tabManager.tab_1}>
											<DetailInfomationTranferRepositoryUser onUpdate={() => this.onUpdate()} openBillTransferRepostitory={() => this.openBillTransferRepository(record)} onSuccess={this.onSuccess} listProductTranfer={record.listProductTranfer} transferRepostitorySelected={record} />
										</Tabs.TabPane>
									</Tabs>
								</>
							),
				...
				dataSource={transferRepositoryDetailResult}
