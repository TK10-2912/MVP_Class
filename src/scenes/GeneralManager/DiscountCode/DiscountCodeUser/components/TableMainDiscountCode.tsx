...
export default class TableMainDiscountCodeUser extends React.Component<IProps> {
	state = {
		isLoadDone: true,
	...
	render() {
		...
		let action: ColumnGroupType<DiscountCodeDto> = {
			title: 'Chức năng', children: [], key: 'action_member_index', className: "no-print center", fixed: 'right', width: 150,
		...
		const columns: ColumnsType<DiscountCodeDto> = [
			...
			{
				title: 'Mệnh giá (VNĐ)', key: 'di_price',
				render: (text: string, item: DiscountCodeDto) => <div>{AppConsts.formatNumber(item.di_price)}</div>
			},
			{
				title: 'Số lượng mã đã dùng', key: 'di_quantity_use',
				render: (text: string, item: DiscountCodeDto) => <div>{item.di_quantity_use.toLocaleString()}</div>
			},
			{
				title: 'Số lượng mã tối đa', key: 'di_quantity_max',
				render: (text: string, item: DiscountCodeDto) => <div>{item.di_quantity_max.toLocaleString()}</div>
			},

			{
				title: ('Mô tả'), key: 'di_desc',
				render: (text: string, item: DiscountCodeDto) => <div style={{ marginTop: "14px" }} dangerouslySetInnerHTML={{ __html: item.di_desc! }}></div>
			},
			{
				...
				render: (text: string, item: DiscountCodeDto) => <>
					{this.props.export === true ?
						...
						<>
							{
								(new Date(item.di_end_at!) < this.current) ?
