...
import TableMainDiscountCodeAdmin from './components/TableMainDiscountCodeAdmin';
import CreateOrUpdateDiscountCodeAdmin from './components/CreateOrUpdateDiscountCodeAdmin';
import ModalExportDiscountCodeAdmin from './components/ModalExportDiscountCodeAdmin';
import ImportExcelDiscountCodeAdmin from './components/ImportExcelDiscountCodeAdmin';

export default class DiscountCodeAdmin extends React.Component<any> {
	state = {
		isLoadDone: false,
	...
	async getAll() {
		...
		await stores.discountCodeStore.getAllForAdmin(this.state.di_code, this.state.di_quantity_max, this.state.di_start_at, this.state.di_end_at, this.state.di_price_down, this.state.di_price_up, this.state.ma_id, this.selectedField, this.state.sort, this.state.skipCount, this.state.pageSize);
			...
			<Card>
				...
				<Row gutter={[8, 8]} align='bottom'>
					...
					<Col {...cssColResponsiveSpan(24, 12, 8, 8, 6, 3)}>
						...
						<SelectEnum
							eNum={eMoney}
							onChangeEnum={(e) => { this.setState({ di_price_down: e }); this.onChangePage(1, this.state.pageSize) }}
					...
					<Col {...cssColResponsiveSpan(24, 12, 8, 8, 6, 3)}>
						...
						<SelectEnum
							...
							onChangeEnum={(e) => { this.setState({ di_price_up: e }); this.onChangePage(1, this.state.pageSize) }}
				...
				<Row>
					...
					{this.state.visibleModalCreateUpdate &&
						<Col {...right}>
							<CreateOrUpdateDiscountCodeAdmin createSuccess={this.createSuccess} discountCodeSelected={this.disCountCodeSelected} onCancel={this.onCancel} disable={(new Date(this.disCountCodeSelected.di_end_at!) < this.current) ? true : false} />
						</Col>
					}
				</Row>
				<ModalExportDiscountCodeAdmin
				...
				<Modal visible={this.state.visibleImportExcel}
					...
					footer={false}
					width={"70%"}>
					<ImportExcelDiscountCodeAdmin onRefreshData={this.onRefreshData} onCancel={() => this.setState({ visibleImportExcel: false })} />
				</Modal>
			</Card>
		)
	}
