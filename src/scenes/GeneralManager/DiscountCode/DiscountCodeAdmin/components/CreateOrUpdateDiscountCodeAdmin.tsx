...
export default class CreateOrUpdateDiscountCodeAdmin extends React.Component<IProps>{
	...
	async componentDidMount() {
		await stores.groupMachineStore.getAllByAdmin(undefined, undefined, undefined, undefined);
	...
	onCreateUpdate = () => {
		...
		form!.validateFields().then(async (values: any) => {
			...
			if (discountCodeSelected?.di_id === undefined || discountCodeSelected.di_id < 0) {
				...
				if(unitData.di_end_at == null)
				{
					unitData.di_end_at  = this.state.di_end_at.toDate();
				}
				else if(unitData.di_start_at == null)
				{
	...
	render() {
		...
		const { treeGroupMachineAdmin } = stores.groupMachineStore;
		return (
			<Card>
				...
				<Form ref={this.formRef} style={{ width: '100%' }} >
					...
					<Form.Item label={'Ngày kết thúc'} {...AppConsts.formItemLayout} name={'di_end_at'} valuePropName='di_end_at'>
						<DatePicker
							...
							disabled={disable}
							disabledDate={(current => current ? current <= moment(this.state.di_start_at).subtract(1, "day") : false)}
						/>
					</Form.Item>
					...
					<Form.Item label={L('Nhóm máy')} {...AppConsts.formItemLayout}>						
						...
						>
							{this.renderTreeNodes(treeGroupMachineAdmin)}
						</Tree>
					</Form.Item >
				</Form>
