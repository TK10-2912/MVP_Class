...
const { confirm } = Modal;
export default class ImportProductRepositoryAdmin extends React.Component<IProps> {
    state = {
        isLoadDone: false,
        checkFile: false,
    }
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        if (!!item[1] && !!item[2] && !!item[3] && !!item[4] && item[5] && !!item[6] && !!item[7]) {
                            ...
                            if (product != undefined) {
                                ...
                                itemCreate.pr_im_name = product.pr_name;
                                itemCreate.pr_im_unit = item[2].toString();
                                itemCreate.pr_im_quantity = item[3] != null ? Number(item[3].toString()) : 0;
                                itemCreate.pr_im_unit_price = item[4] != null ? Number(item[4].toString()) : 0;
                                itemCreate.pr_im_unit_quydoi = product.pr_unit;
                                itemCreate.pr_im_quantity_quydoi = item[6] != null ? Number(item[6].toString()) : 0;
                                itemCreate.pr_im_total_money = itemCreate.pr_im_quantity;
                                itemCreate.pr_im_total_money = totalMoney;
    ...
    render() {
        const columns: ColumnsType<ProductImportDto> = [
            ...
            { title: "Đơn giá", key: "pr_im_unit_price", dataIndex: "pr_im_unit_price", render: (text: number, item: ProductImportDto) => <div> {AppConsts.formatNumber(item.pr_im_unit_price) + " VND"} </div> },
            { title: "Đơn vị quy đổi", key: "pr_im_unit_quydoi", dataIndex: "pr_im_unit_quydoi", render: (text: number, item: ProductImportDto) => <div> {item.pr_im_unit_quydoi} </div> },
            { title: "Số lượng quy đổi", key: "pr_im_quantity_quydoi", dataIndex: "pr_im_quantity_quydoi", render: (text: string, item: ProductImportDto) => <div> {item.pr_im_quantity_quydoi} </div> },
            { title: "Thành tiền", key: "pr_im_total_money", dataIndex: "pr_im_total_money", render: (text: number, item: ProductImportDto) => <div> {AppConsts.formatNumber(item.pr_im_quantity * item.pr_im_unit_price) + " VND"} </div> },
        ];
        return (
            <Modal
