...
import ImportRepositoryTable from './components/ImportRepositoryTable';
...
export default class ImportRepositoryAdmin extends AppComponentBase<IProps> {
	state = {
		isLoadDone: true,
							...
							<Row gutter={[8, 8]} align='bottom'>
								...
								<Col {...cssColResponsiveSpan(24, 12, 12, 6, 4, 3)}>
									...
									<SelectedSupplierMultiple
										...
										onChangeSupplier={async (value) => { await this.setState({ su_id_list: value }); this.handleSubmitSearch() }}
									></SelectedSupplierMultiple>
								</Col>
								{this.isGranted(AppConsts.Permission.Pages_Manager_General_Admin_ImportRepository) &&
								...
								</Col>
								{
									...
									this.isGranted(AppConsts.Permission.Pages_Manager_General_Admin_ImportRepository) ?
										...
										<Col {...cssColResponsiveSpan(24, 24, 12, 12, 24, 6)} style={{ display: "flex", flexWrap: "wrap", justifyContent: "end", gap: 8 }}>
											{this.isGranted(AppConsts.Permission.Pages_Manager_General_ImportRepository_Create) &&

												<Button title='Nhập hàng' type="primary" icon={<PlusOutlined />} onClick={() => { this.setState({ visibleImportRepositoryDetail: true }); this.importRepositorySelected = new ImportRepositoryDto() }}>{this.shouldChangeText() && 'Nhập hàng'}</Button>
											}
