...
export default class TableRFIDAdmin extends React.Component<IProps> {
	...
	onAction = async (item: RfidDto, action: EventTable, checked?: boolean) => {
		...
		if (hasAction !== undefined && hasAction === true && actionTable !== undefined) {
			await actionTable(item, action, checked);
		}
	}
	render() {
		...
		let action: ColumnGroupType<RfidDto> = {
			title: L('Chức năng'), children: [], key: 'action_Supplier_index', className: "no-print center", width: 150,
			render: (text: string, item: RfidDto) => (
				<Space>
					{isGranted(AppConsts.Permission.Pages_Manager_General_RFID_ChangeMoney) &&
						<Button
							type="primary" icon={<EditOutlined />} title={L('Điều chỉnh số dư RFID')}
							size='small'
		...
		const columns: ColumnsType<RfidDto> = [
			...
			// { title: L('Người sở hữu'), sorter: (this.props.export?false:true), dataIndex: "us_id_owner", key: 'us_id_owner', render: (text: string, item: RfidDto) => <div> {stores.sessionStore.getUserNameById(item.us_id_owner)}</div> },
			...
			{ title: L('Ngày tạo'), sorter: !this.props.export, dataIndex: 'rf_created_at', key: 'su_desc', render: (text: string, item: RfidDto) => <div> {moment(item.rf_created_at).format("DD/MM/YYYY - HH:mm:ss")}</div> },
			{
				title: 'Kích hoạt',
				dataIndex: 'isActive',
				key: 'isActive',
				width: this.props.hasAction === true ? 150 : 0,
				render: (text: string, item: RfidDto) => (
					<>
						{this.props.export ? (
							...
							<div
								onClick={(event) => {
									event.stopPropagation(); // Stop propagation for the entire column
								}}
							>
								<Switch
									checked={item.rf_is_active}
									onClick={(checked: boolean) => {
										this.onAction(item, EventTable.ChangeStatus, checked);
									}}
								/>
							</div>
						)}
		...
		return (
			<Table
				...
				onRow={(record) => {
					return {
						onDoubleClick: (event: any) => {
							// Prevent double-click if it's from the "Kích hoạt" column (Switch)
							if (event.target.closest('.ant-switch')) {
								return; // Don't trigger the double-click action if clicked inside the switch
							}
				...
				rowClassName={(record) => (this.state.rf_id_selected === record.rf_id) ? "bg-click" : "bg-white"}
