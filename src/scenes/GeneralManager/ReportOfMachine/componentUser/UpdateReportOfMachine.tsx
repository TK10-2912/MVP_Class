...
import rules from './UpdateReportOfMachine.validation';
...
export default class UpdateReportOfMachine extends React.Component<IProps>{
            ...
            <Card>
                ...
                <Row>
					<Form ref={this.formRef} style={{ width: '100%'}}>
						...
						<Form.Item label="Mã báo cáo" {...AppConsts.formItemLayout} name={"re_code"}>
						...
						<Form.Item label="Mức nghiêm trọng" {...AppConsts.formItemLayout} name={"re_level"} valuePropName='data'>
						...
						<Form.Item label="Hiển thị báo cáo" {...AppConsts.formItemLayout} name={"re_display"} valuePropName='data'>
							<h3 style={{marginTop:"3px"}}>{reportOfMachineSelected.re_display}</h3>
						</Form.Item>

						<Form.Item label="Trạng thái" {...AppConsts.formItemLayout} name={"re_status"} rules={rules.rule}>
						<SelectEnum eNum={eReportStatus} enum_value={this.props.reportOfMachineSelected.re_status} onChangeEnum={async (value: number) => {await this.setState({re_status: value}); await this.formRef.current!.setFieldsValue({ re_status: value });}}/>
						</Form.Item>
