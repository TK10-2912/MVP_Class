...
export default class TableReportOfMachine extends React.Component<IProps> {
	state = {
		isLoadDone: true,
	...
	render() {
		const { actionTable, reportOfMachineListResult, pagination, noPrint } = this.props;
		let action: any = {
		...
		const columns: ColumnsType<ReportOfMachineDto> = [
			{ title: "STT", width: 50, key: "stt_drink_index", render: (text: string, item: ReportOfMachineDto, index: number) => <div>{pagination !== false ? pagination.pageSize! * (pagination.current! - 1) + (index + 1) : index + 1}</div> },
			...
			{ title: "Ghi chú", key: "re_note_index", render: (text: string, item: ReportOfMachineDto) => <div style={{ marginTop: "14px" }} dangerouslySetInnerHTML={{ __html: item.re_note! }}></div> },
		];
		...
		return (
			<Table
				...
				scroll={this.props.noPrint === true ? { x: undefined, y: undefined } : { x: 1500, y: 600 }}
				columns={columns}
				size={'middle'}
				bordered={true}
