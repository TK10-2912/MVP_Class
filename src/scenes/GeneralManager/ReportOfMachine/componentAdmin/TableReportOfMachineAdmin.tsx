import { CheckCircleOutlined, EnvironmentOutlined, EyeOutlined, SendOutlined } from "@ant-design/icons";
...
export default class TableReportOfMachineAdmin extends React.Component<IProps> {
	...
	render() {
		...
		let action: ColumnGroupType<ReportOfMachineDto> = {
			title: "Chức năng", key: "action_reportmachine_index", className: "no-print", fixed: "right", width: 100, children: [],
		...
		const columns: ColumnsType<ReportOfMachineDto> = [
			{ title: "STT", key: "stt_drink_index", width: 50, render: (_: string, item: ReportOfMachineDto, index: number) => <div>{pagination !== false ? pagination.pageSize! * (pagination.current! - 1) + (index + 1) : index + 1}</div> },
			{
				title: "Mã đơn hàng",
				key: "list_product",
				className: "hoverCell",
				...
				render: (_: string, item: ReportOfMachineDto) => (
			...
			{
				title: "Nhóm máy", sorter: false, dataIndex: "ma_gr_id", key: "ma_gr_id", render: (_: number, item: ReportOfMachineDto) => <div title="Chi tiết nhóm máy">
			...
			{
				title: "Máy bán nước", sorter: false, dataIndex: "ma_id", key: "ma_id", render: (_: number, item: ReportOfMachineDto) => <div title="Chi tiết máy">
			...
			{
				title: "Người vận hành", sorter: false, dataIndex: "us_id_operator", key: "us_id_report_index",
				render: (_: number, item: ReportOfMachineDto) =>
					<div>
						{item.machine != undefined && item.machine.us_id_operator != undefined ? stores.sessionStore.getUserNameById(item.machine.us_id_operator) : 'khoaliennganh'}
					</div>
			},

			{ title: "Mô tả báo cáo", sorter: false, dataIndex: "re_display", key: "re_display_index", render: (_: string, item: ReportOfMachineDto) => <div>{item.re_display}</div> },
			{
				title: "Trạng thái", sorter: false, dataIndex: "re_status", key: "re_status_index", render: (_: number, item: ReportOfMachineDto) => {
					if (is_Printed === false) {
						return <div>
			...
			{ title: "Ghi chú", ellipsis: { showTitle: false }, key: "re_note_index", render: (_: string, item: ReportOfMachineDto) => <div style={{ marginTop: "14px", overflow: 'hidden', textOverflow: 'ellipsis' }} dangerouslySetInnerHTML={{ __html: item.re_note! }}></div> },
			{ title: "Ngày tạo", width: 100, sorter: true, dataIndex: "re_created_at", key: "re_created_at", render: (_: number, item: ReportOfMachineDto) => <div>{moment(item.re_created_at).format("DD/MM/YYYY HH:mm:ss")}</div> },
			{
		...
		return (
			<>
				<Table
					scroll={this.props.is_Printed === true ? { x: undefined, y: undefined } : { x: 1200, y: 600 }}
					...
					footer={(record) => {
						...
						return (//tải xuống không nhận classname, dùng css inline
