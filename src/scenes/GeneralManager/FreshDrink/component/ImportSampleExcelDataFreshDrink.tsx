...
import { ImportFreshDrinkInput, } from '@src/services/services_autogen';
...
const { confirm } = Modal;
export default class ImportSampleExcelDataFreshDrink extends React.Component<IProps> {
    state = {
        isLoadDone: false,
        checkFile: false,
    }

    dataExcel: ImportFreshDrinkInput[] = [];
    dicImage: {} = {};
    nameForder: string = "";
    nameFile: string = "";
            ...
            await readXlsxFile(item).then(async (rows) => {
                if (rows !== undefined && rows.length > 1) {
                    for (let i = 1; i < rows.length; i++) {
                        let itemCreate: ImportFreshDrinkInput = new ImportFreshDrinkInput();
                        let item = rows[i];
                        if (!item[2] || !item[3] || !item[4] || !item[5] || isNaN(Number(item[4])) || isNaN(Number(item[5]))) {
                            ...
                            itemCreate.fr_dr_image = item[1] != null ? this.dicImage[item[1].toString()] : '';
                            itemCreate.fr_dr_name = item[2].toString();
                            itemCreate.su_name = item[3].toString();
                            itemCreate.fr_dr_price = item[4] != null ? Number(item[4]) : 0;
                            itemCreate.fr_dr_capacity = item[5] != null ? Number(item[5]) : 0;
    ...
    async createListLSC() {
        ...
        confirm({
            ...
            async onOk() {
                ...
                await stores.freshDrinkStore.createListFreshDrink(self.dataExcel);
    ...
    render() {
        const columns = [
            { title: L('STT'), dataIndex: '', key: 'no_drink_index', render: (text: string, item: any, index: number) => <div>{index + 1}</div> },
            {
                title: "Ảnh minh họa", dataIndex: 'dr_name', key: "dr_name", render: (text: string, item: ImportFreshDrinkInput) => <div>
                    {<Avatar src={item.fr_dr_image === undefined ? "" : item.fr_dr_image} />} </div>
            },
            { title: "Tên sản phẩm", dataIndex: 'dr_name', key: "dr_name", render: (text: string, item: ImportFreshDrinkInput) => <div> {item.fr_dr_name} </div> },
            { title: "Nhà cung cấp", dataIndex: 'su_name', key: "su_name", render: (text: string, item: ImportFreshDrinkInput) => <div> {item.su_name} </div> },
            { title: "Giá thành (VNĐ)", dataIndex: 'dr_price', key: "dr_price", render: (text: string, item: ImportFreshDrinkInput) => <div> {AppConsts.formatNumber(item.fr_dr_price)}</div> },
            { title: "Dung tích (ml)", dataIndex: 'dr_desc', key: "dr_desc", render: (text: string, item: ImportFreshDrinkInput) => <div>{AppConsts.formatNumber(item.fr_dr_capacity)}</div> },
        ];
        return (
            <>
                <Row gutter={[8, 8]}>
                    ...
                    >
                        ...
                        <Button style={{ color: 'red', backgroundColor: 'floralwhite' }} title={'Dữ liệu mẫu'} target="_blank" href={process.env.PUBLIC_URL + "/sample_import/che_pham_khong_bao_bi_mau.xlsx"}>
                            File mẫu
                        </Button>
                    </Col>
                ...
                <Row gutter={[8, 8]}>
                    ...
                    <Col {...cssColResponsiveSpan(24, 24, 12, 12, 12, 12)}>
                        ...
                        {Object.getOwnPropertyNames(this.dicImage).length > 0 ?
                            ...
                            :
                            <Tag color="magenta" style={{ marginTop: 10 }}>
                                Chưa tải lên folder
                            </Tag>
                        }
                    </Col>
