...
import FreshDrinkTable from './FreshDrinkTable';
...
export default class ModalExportFreshDrink extends AppComponentBase<IProps> {
	...
	exportToExcel = async () => {
		const { freshDrinkListResult } = this.props;
		...
		freshDrinkListResult.forEach((item, index) => {
		...
		return (
			<Modal
				...
				title={
					<Row>
						<Col span={12}>
							Xuất danh sách sản phẩm không có bao bì
						</Col>
						<Col span={12} style={{ textAlign: 'end' }}>
							<ActionExport
								nameFileExport={'fresh_drink' + ' ' + moment().format('DD_MM_YYYY')}
								idPrint="fresh_drink_print_id"
			...
			>
				<Col ref={this.setComponentRef} span={24} style={{ marginTop: '10px' }} id="fresh_drink_print_id">
					<TitleTableModalExport title='Danh sách sản phẩm không có bao bì'></TitleTableModalExport>
					<FreshDrinkTable
