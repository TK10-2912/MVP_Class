...
import ModalExportCashDetailListBill from './ModalExportCashReconcoleUserDetailAdmin';
...
export default class TableBillingCashViewAdmin extends React.Component<IProps> {
    state = {
        isLoadDone: true,
    ...
    componentDidMount() {
        ...
        const { listIdBill, billListResult } = this.props;
        this.listDataBill = billListResult != undefined ? billListResult.filter(item=> listIdBill?.includes(item.bi_id)) : [];
    ...
    render() {
        ...
        const columns: ColumnsType<BillingDto> = [
            ...
            {
                title: "Trạng thái hoàn tiền", width: "20%", className: 'pointHover', key: "ma_id_bill_index", render: (text: string, item: BillingDto) => <div >
        ...
        return (
            <>
                ...
                <Table
                    ...
                    scroll={(hasAction != undefined && hasAction === true) ? { x: 1400, y: 600 } : { x: undefined, y: undefined }}
                    columns={columns}
                    size={'middle'}
                    bordered={true}
                ...
                />
                  {this.state.visibleExport &&
                    <ModalExportCashDetailListBill
                        visible={this.state.visibleExport}
                        listId={this.props.listIdBill != undefined ? this.props.listIdBill : []}
