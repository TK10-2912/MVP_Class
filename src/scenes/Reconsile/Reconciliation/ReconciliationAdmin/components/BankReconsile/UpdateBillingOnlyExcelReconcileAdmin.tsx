...
export default class UpdateBillingOnlyExcelReconcileAdmin extends React.Component<IProps> {
    state = {
        isLoadDone: true,
    ...
    async getAllLog() {
        await this.setState({ bi_id_selected: this.props.onlyExcelReconcile.ex_code })
        await stores.reconcileLogsStore.getAll(this.props.onlyExcelReconcile.ex_code?.split("_")[0], undefined, undefined);
    ...
    initData = async (input: ExcelReconcileDto) => {
        ...
        if (input !== undefined && input.ex_code !== undefined) {
            ...
            this.formRef.current!.setFieldsValue({ ...input, reconcile_reason: input.ex_reconcile_reason,fi_id_list:this.listAttachmentItem });
        }
        else {
            ...
            <Card>
                ...
                <Row style={{ marginTop: 10 }}>
                    <Form ref={this.formRef} style={{ width: "100%" }}>
                        <Form.Item label="Trạng thái đối soát" {...AppConsts.formItemLayoutTitleSmall} name={'ex_reconcile_status'}  >
