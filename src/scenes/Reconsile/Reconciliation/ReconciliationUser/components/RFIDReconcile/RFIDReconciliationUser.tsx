...
import ModalImportRFIDReconciliationUser from './ModalImportRFIDReconciliationUser';
import TableReconcileRFIDUser from './TableReconcileRFIDUser';
import UpdateBillingRFIDReconcile from './UpateBillingRFIDReconcile';
import TableListRFIDBillingOnlyInExcelUser from './TableListRFIDBillingOnlyInExcelUser';
import UpdateBillingOnlyExcelRFIDReconcile from './UpdateBillingOnlyExcelRFIDReconcile';
import ModalExportRFIDReconcolelUser from './ModalExportRFIDReconcoleUser';
import ModalExportRFIDReconcoleUserDetail from './ModalExportRFIDReconcoleUserDetail';
import TableBillingDetailRFID from './TableBillingDetailRFID';
import TableBillingRFIDSystemUser from './TableListBillRFIDSystem';
import { isGranted } from '@src/lib/abpUtility';
export interface Iprops {
    tab?: string;
}
const { confirm } = Modal;
export default class RFIDReconciliationUser extends React.Component<Iprops> {
    state = {
    ...
    getAll = async () => {
        ...
        await Promise.all([
            stores.reconcileStore.getAllRFIDReconcile(this.state.start_date, this.state.end_date, this.state.gr_ma_id, this.state.ma_id_list, this.state.skipCount, this.state.pageSize),
        ])
        ...
        return (
            <Card>
                    ...
                    <Col span={12} style={{ textAlign: "end", marginBottom: 10 }}>
                        {
                            ...
                            <Button style={{ marginRight: "10px" }} type='primary' onClick={() => this.setState({ visibleImportExcel: true })}>{"Đối soát RFID"}</Button>
                        } 
                        ...
                        <Row gutter={10}>
                            ...
                            <Col {...right} style={{ marginTop: 105 }}>
                                {this.state.visibleUpdateStatusReconcile &&
                                    <UpdateBillingRFIDReconcile onCancel={() => this.setState({ visibleUpdateStatusReconcile: false })} onSuccess={() => this.getSuccess()} bill_select={this.billingSelected} logReconcile={reconcileLogsListDto[0]} reconcileSelect={this.reconcileDto.rec_id} />
                                }
                            </Col>
                        </Row>
                        <Row gutter={10}>
                            <Col {...leftOnlyExcel}>
                                <TableListRFIDBillingOnlyInExcelUser
                                    reconcileListDto={reconcileRFIDListDto}
                                    isLoadDone={this.state.isLoadDone}
                            ...
                            <Col {...rightOnlyExcel} style={{ marginTop: 105 }}>
                                {this.state.visibleUpdateStatusReconcileOnlyExcel &&
                                    <UpdateBillingOnlyExcelRFIDReconcile onlyExcelReconcile={this.onlyExcelReconcile} logReconcile={reconcileLogsListDto[0]} reconcileSelect={this.reconcileDto} onCancel={() => this.setState({ visibleUpdateStatusReconcileOnlyExcel: false })} onSuccess={() => this.getSuccess()} />
                                }
                            </Col>
                        </Row>
                    </>
                ...
                <Modal
                    ...
                    width={"85%"}
                >
                    <TableBillingDetailRFID onCancel={() => this.setState({ visivleModalRfidBillDetail: false })} ma_id={this.reconcileDto.ma_id} rfid_id_list={this.reconcileDto.listRfidLogsId} billing={billListResult} />
                </Modal>
                <Modal
                    ...
                    width={"85%"}
                >
                    <ModalImportRFIDReconciliationUser
                        onCancel={() => { this.setState({ visibleImportExcel: false }) }}
                ...
                {this.state.visibleExportExcelDetail &&
                    <ModalExportRFIDReconcoleUserDetail
