...
import { Col, Row, Button, Card, Input, DatePicker, Select, Space, Modal, } from 'antd';
import { stores } from '@stores/storeInitializer';
import { L, isGranted } from '@lib/abpUtility';
import { DeleteOutlined, ExportOutlined, ReconciliationOutlined, SearchOutlined } from '@ant-design/icons';
...
import ModalExportRefundAdmin from './components/ModalExportRefundAdmin';
import TableRefundAdmin from './components/TableRefundAdmin';
...
import { eFormatPicker } from '@src/components/Manager/StatisticSearchProductAdmin';
import CreateRefundAdmin from './components/CreateRefundAdmin';

const { RangePicker } = DatePicker;
const { Option } = Select;

export default class RefundAdmin extends AppComponentBase {
	state = {
		isLoadDone: true,
	...
	async getAll() {
		await stores.refundStore.getAllByAdmin(this.state.us_id_list, this.state.ref_reason_type, this.state.ref_status, this.state.ref_refund_type, this.state.bi_code, this.state.ref_nameBank, this.state.ref_from, this.state.ref_to, this.state.gr_ma_id, this.state.ma_id_list, this.selectedField, this.state.sort, this.state.skipCount, this.state.pageSize);
	...
	async componentDidMount() {
		...
		if (!!biCode) {
			await this.setState({ bi_code: biCode });
		}
		...
		const { billListResult } = await stores.billingStore;
		this.billingDto = billListResult;
		await this.getAll();
	}
	handleSubmitSearch = async () => {
		...
		let end_date = !!this.state.rangeDatetime?.[1] ?
			...
			undefined
		await this.setState({ ref_from: start_date, ref_to: end_date })
	...
	actionTable = (item: RefundDto, event: EventTable) => {
		if (event === EventTable.Edit) {
			this.refundSelected.init(item);
		}
		if (event === EventTable.SetErrorStatus) {
			stores.refundStore.updateStatusError(item.ref_id)
		}
	}

	shouldChangeText = () => window.innerWidth >= 576 && window.innerWidth <= 992;
		...
		return (
			<Card>
					...
					{isGranted(AppConsts.Permission.Pages_Reconcile_Refund_Export) &&
						<Col {...cssColResponsiveSpan(24, 12, 12, 12, 12, 12)} style={{ display: "flex", justifyContent: "end" }}>
							<Space>
								...
								<Button type='primary' icon={<ExportOutlined />} title='Xuất dữ liệu' onClick={() => { this.setState({ visibleExportRefund: true }) }} >Xuất dữ liệu</Button>
							</Space>
						</Col>
					}
				</Row>
				<Row align='bottom' gutter={[12, 12]}>
					...
					<Col xs={24} sm={12} md={8} lg={6} xl={4}>
						...
						<SelectedGroupMachine 
							...
							onChangeGroupMachine={value => this.setState({ gr_ma_id: value })} 
						/>
					</Col>
					<Col xs={24} sm={12} md={8} lg={6} xl={4}>
					...
					<Col xs={24} sm={12} md={8} lg={6} xl={4}>
						...
						<SelectedBank 
							...
							onChangeBank={(value: string) => { 
								this.setState({ ref_nameBank: value }); 
								this.handleSubmitSearch() 
							}} 
						/>
					</Col>
					...
					<Col xs={24} sm={24} md={24} lg={24} xl={4} style={{ display: "flex", justifyContent: "start", alignItems: "center", marginTop: 8 }}>
						<Button 
				...
				<Row style={{ marginTop: '10px' }}>
					<Col>
						<TableRefundAdmin
							...
							onCreateUpdateSuccess={() => this.onChangePage(1, this.state.pageSize)}
							onCancel={() => this.setState({ visibleModalCreateUpdate: false })}
							checkExpand={this.state.visibleModalCreateUpdate}
							changeColumnSort={this.changeColumnSort}
							actionTable={this.actionTable}
							refundResult={refundListDto}
							billingResult={this.billingDto}
							hasAction={!this.state.visibleExportRefund}
				...
				<Modal
					...
					maskClosable={true}
				>
					<CreateRefundAdmin onCreateUpdateSuccess={() => this.onChangePage(1, this.state.pageSize)} onCancel={() => this.setState({ visibleModalCreateUpdate: false })} />
				</Modal>
				<ModalExportRefundAdmin
