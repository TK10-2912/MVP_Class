...
import { DeleteFilled, EditOutlined, EyeOutlined, InfoCircleOutlined, } from '@ant-design/icons';
import { L, isGranted } from '@lib/abpUtility';
import { Avatar, Button, Image, Space, Switch, Table, Tag } from 'antd';
...
export interface IProps {
	...
	hasAction?: boolean;
	doId?: number;
	// rowSelection?: TableRowSelection<RefundDto>
	export?: boolean;
...
export default class TableRefundUser extends React.Component<IProps> {
	...
	render() {
		...
		let action: ColumnGroupType<RefundDto> = {
			...
			render: (item: RefundDto) => (
				<Space>
					{isGranted(AppConsts.Permission.Pages_Reconcile_Refund_Update) &&
						<Button
							type="primary" icon={item.ref_refund_at != null ? <EyeOutlined /> : <EditOutlined />}
							title={L('Xem thông tin hoàn tiền đơn hàng')}
							size='small'
		...
		const columns: ColumnsType<RefundDto> = [
			...
			{
                title: "Nhóm máy", key: "nhom_may", width: 150, render: (text: string, item: RefundDto) => <div>
                    <Link to={"/general/machine/?gr_id=" + stores.sessionStore.getIDGroupUseMaId(item.ma_id)} onDoubleClick={() => this.onAction(item, EventTable.View)} >
			...
			{
				title: "Tên máy", key: "ten_may", width: 150, render: (text: string, item: RefundDto) => <div>
					<Link to={"/general/machine/?machine=" + stores.sessionStore.getNameMachines(item.ma_id)} onDoubleClick={() => this.onAction(item, EventTable.View)}>
			...
			{ title: L('Ảnh giao dịch'), dataIndex: 'ref_reason_type', key: 'su_desc', width: 120, render: (text: string, item: RefundDto) => <div>{item.fi_id_list != undefined && item.fi_id_list!.length > 0 ? <img src={(this.getFile(item.fi_id_list[0].id))} height={100} width={100}></img> : <Image src={process.env.PUBLIC_URL + "/image/no_image.jpg"} height={100} width={100} />}</div> },
			...
			{ title: L('Chủ tài khoản'), dataIndex: 'ref_nameAccountBank', key: 'su_desc', width: 150, render: (text: string, item: RefundDto) => <div> {item.ref_nameAccountBank}</div> },
			...
			{ title: L('Trạng thái hoàn tiền'), dataIndex: 'ref_refund_at',width: 120,  key: 'su_desc', render: (text: string, item: RefundDto) => <div> {(item.ref_refund_at != null ? <Tag color='green'>Đã hoàn tiền</Tag> : <Tag color='red'>Chưa hoàn tiền</Tag>)}</div> },
			{ title: 'Phương thức thanh toán', dataIndex: 'ref_refund_type', key: 'su_desc', render: (text: string, item: RefundDto) => <div> {valueOfePaymentMethod(item.ref_refund_type)}</div> },
