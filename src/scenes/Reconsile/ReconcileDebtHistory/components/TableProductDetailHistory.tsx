...
export default class TableProductDetailHistory extends AppComponentBase<IProps> {
	state = {
		isLoadDone: false,
		...
		const columns: ColumnsType<ProductImportDto> = [
			{
				...
				render: (text: string, item: ProductImportDto, index: number) => <div>{index + 1}</div>
			},
			{
				title: "Ảnh",
				width: 120,
			...
			{
				title: "Trạng thái", key: "pr_name", width: "23%",
				render: (text: string, item: ProductImportDto, index: number) => <div>
					{
