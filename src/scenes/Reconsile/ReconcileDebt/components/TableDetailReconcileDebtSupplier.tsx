...
export default class TableDetailReconcileDebtSupplier extends React.Component<IProps> {
    state = {
        isLoadDone: true,
        ex_code: undefined,
                ...
                return (
                    <>
                        <Space>
                            {
                                (() => {
                                    const listResult = record ? record.listProductImport!.filter(item => item.pr_im_reconcile_status === eBillReconcileStatus.DONE.num) : [];
                                    return (
                                        <>
                                            {record.rec_status == eReconcileDebtStatus.WAIT.num && listResult.length == record.listProductImport?.length && (
                                                <Button
                                                    type="primary" icon={<CheckSquareOutlined />} title={L('Xác nhận đối soát')}
                                                    size='small'
                                                    onClick={() => this.actionTableSuppplier(record, EventTable.Accept)}
                                                ></Button>
                                            )}
                                        </>
                                    );
        ...
        const columns: ColumnsType<ReconcileSupplierDebtDetailDto> = [
            ...
            },
            {
                ...
                sorter: (a: ReconcileSupplierDebtDetailDto, b: ReconcileSupplierDebtDetailDto) =>
                    (a.listProductImport?.length || 0) - (b.listProductImport?.length || 0),
            ...
            },
            {
                ...
                sorter: (a: ReconcileSupplierDebtDetailDto, b: ReconcileSupplierDebtDetailDto) =>
                    (a.listProductImport?.reduce((total, currentTotal) => total + currentTotal.pr_im_quantity, 0) || 0) -
                    (b.listProductImport?.reduce((total, currentTotal) => total + currentTotal.pr_im_quantity, 0) || 0),
                render: (text: string, item: ReconcileSupplierDebtDetailDto, index: number) =>
                    <div>{AppConsts.formatNumber(item.listProductImport?.reduce((total, currentTotal) => total + currentTotal.pr_im_quantity, 0))}</div>
            },
            {
            ...
            { title: "Thời gian tạo đối soát", sorter: (a, b) => moment(a.rec_to).unix() - moment(b.rec_to).unix(), key: "ex_reconcile_at", render: (text: string, item: ReconcileSupplierDebtDetailDto) => <div> {moment(item.rec_from).format("DD/MM/YYYY")} - {moment(item.rec_to).format("DD/MM/YYYY")} </div> },
        ]
        ...
        return (
            <div>
                ...
                <Table
                    ...
                    dataSource={(this.state.bi_reconcile_status_search.length > 0 || this.state.rec_code_search != undefined) ? this.listDataReconcileFill : this.listDataReconcile}
