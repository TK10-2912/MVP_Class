...
export default class ThongKeDoanhThuTheoMayAdmin extends AppComponentBase {
    ...
    onSearch = (inputSearch: SearchInputAdmin) => {
    ...
    render() {
        ...
        const columns: ColumnsType<StatisticBillingOfMachineDto> = [
            { title: "STT", className: "start", key: "stt", width: 50, render: (text: string, item: StatisticBillingOfMachineDto, index: number) => <div>{this.state.pageSize! * (this.state.currentPage! - 1) + (index + 1)}</div> },
            {
            ...
            {
                title: "Người sở hữu", width: 150, key: "us_id_owner", sorter: (a, b) => a.totalQuantity - b.totalQuantity, render: (text: string, item: StatisticBillingOfMachineDto) => <div>
...

            {
                ...
                children: [
                    { title: "Có bao bì", width: 150, sorter: (a, b) => a.quantityDrink - b.quantityDrink, key: "pr_name", render: (text: string, item: StatisticBillingOfMachineDto) => <div>{AppConsts.formatNumber(item.quantityDrink)}</div> },
                    { title: "Không bao bì (ml)", width: 150, sorter: (a, b) => a.quantityFreshDrink - b.quantityFreshDrink, key: "pr_name", render: (text: string, item: StatisticBillingOfMachineDto) => <div>{AppConsts.formatNumber(item.quantityFreshDrink)}</div> },
                ]
            },
            {
            ...
            { title: "Tổng số lượng đơn", width: 125, key: "total_number", sorter: (a, b) => a.totalQuantity - b.totalQuantity, render: (text: string, item: StatisticBillingOfMachineDto) => <div>{AppConsts.formatNumber(item.totalQuantity)}</div> },
            { title: "Tổng doanh thu (VNĐ)", width: 125, key: "total", sorter: (a, b) => a.totalMoney - b.totalMoney, render: (text: string, item: StatisticBillingOfMachineDto) => <div>{AppConsts.formatNumber(item.totalMoney)}</div> },
        ];
            ...
            <Card >
                <Row>
                    <Col {...cssColResponsiveSpan(24, 24, 24, 24, 24, 24)}>
                        <StatisticSearchByAdmin
                            onSearchStatistic={(input) => this.onSearch(input)}
                ...
                <div id='thongkedoanhthutheomay' ref={this.setComponentRef}>
                    ...
                    <Table
                        ...
                        columns={columns}
                        scroll={this.state.noScrollReport ? { x: undefined } : { x: 1670 }}
