...
import StatisticSearchByPriceUnitInputAdmin from '@src/components/Manager/StatisticSearchPriceAdmin';
...
export default class BaoCaoTheoGiaBanAdmin extends AppComponentBase {
    ...
    searchStatistic(input: SearchPriceUnitInputAdmin) {
        this.inputSearch = input;
        this.onChangePage(1, this.state.pageSize);
    }
    getAll = async () => {
        await stores.statisticStore.statisticOfPriceUnitbyAdmin(this.inputSearch);
    ...
    render() {
        ...
        const { listStatisticOfPriceUnit } = stores.statisticStore;
        const columns = [
            { title: "STT", key: "stt", width: 50, render: (_: string, item: StatisticOfPriceUnitDto, index: number) => <div>{this.state.pageSize! * (this.state.currentPage! - 1) + (index + 1)}</div> },
            { title: "Giá bán", key: "money", sorter: (a, b) => a.price - b.price, width: 130, render: (_: string, item: StatisticOfPriceUnitDto) => <div>{AppConsts.formatNumber(item.price)}</div> },
            {
        ...
        return (
            <Card >
                <Row gutter={[8, 8]}>
                    <Col span={24}>
                        <StatisticSearchByPriceUnitInputAdmin getTypeDate={(typeDate) => this.getTypeDate(typeDate)} onSearchStatistic={(value) => this.searchStatistic(value)}></StatisticSearchByPriceUnitInputAdmin>
                    </Col>
                                    ...
                                    if (!!start_date) {
                                        return (
                                            formattedStartDate === formattedEndDate || end_date === undefined
                                                ? <>{"BÁO CÁO THEO GIÁ BÁN NGÀY " + formattedStartDate}</>
                                                : <>{"BÁO CÁO THEO GIÁ BÁN TỪ NGÀY " + formattedStartDate + " ĐẾN NGÀY " + formattedEndDate}</>
                                        );
                                    }
                                    ...
                                    if (!!start_date) {
                                        return (
                                            formattedStartMonth === formattedEndMonth || end_date === undefined
                                                ? <>{"BÁO CÁO THEO GIÁ BÁN THÁNG " + formattedStartMonth}</>
                                                : <>{"BÁO CÁO THEO GIÁ BÁN TỪ THÁNG " + formattedStartMonth + " ĐẾN THÁNG " + formattedEndMonth}</>
                                        );
                                    }
                                    ...
                                    if (!!start_date) {
                                        return (
                                            formattedStartYear === formattedEndYear || end_date === undefined
                                                ? <>{"BÁO CÁO THEO GIÁ BÁN NĂM " + formattedStartYear}</>
                                                : <>{"BÁO CÁO THEO GIÁ BÁN TỪ NĂM " + formattedStartYear + " ĐẾN NĂM " + formattedEndYear}</>
                                        );
                                    }
                                }

                                return <> BÁO CÁO THEO GIÁ BÁN</>;
                            })()
                        }
                    </h2>
                    <Table
