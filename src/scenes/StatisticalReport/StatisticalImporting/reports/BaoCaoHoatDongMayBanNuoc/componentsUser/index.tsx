...
import TableHoatDongMayBanNuocUser from './TableHoatDongMayBanNuocUser';
import StatisticSearch from '@src/components/Manager/StatisticSearch';
import AppConsts, { pageSizeOptions } from '@src/lib/appconst';

export default class BaoCaoHoatDongMayBanNuocUser extends AppComponentBase {
    ...
    getAll = async () => {
        ...
        await stores.statisticStore.statisticImportSellRemainProduct(this.inputSearch)
    ...
    render() {
        const self = this;
        const { listImportSellRemainProduct, totalCountRemainProduct } = stores.statisticStore;
        const newList = listImportSellRemainProduct.slice(0, -1);
        let dateRangeText = "";
        ...
        return (
            <Card >
                <Row >
                    <Col span={24}>
                        <StatisticSearch getTypeDate={this.getTypeDate} onSearchStatistic={(value) => this.searchStatistic(value)}></StatisticSearch>
                    </Col>
                </Row>
                <Row gutter={[8, 8]}>
                ...
                <div id='baocaohoatdongmaybannuoc' ref={this.setComponentRef}>
                    <TableHoatDongMayBanNuocUser
                        listStatistic={newList}
                        totalSatistic={listImportSellRemainProduct[listImportSellRemainProduct.length - 1]}
