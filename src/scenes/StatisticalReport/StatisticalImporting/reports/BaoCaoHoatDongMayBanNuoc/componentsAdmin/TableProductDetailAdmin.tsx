...
export default class TableProductDetailAdmin extends AppComponentBase<Iprops> {
    ...
    render() {
        const  gr_name= stores.sessionStore.getNameGroupUseMaId(stores.sessionStore.getIDMachineUseName(this.props.ma_name))
        ...
        const columns: ColumnsType<ImportSellRemainProductDto> = [
...

            {
                title: "Ảnh", key: "money", render: (text: string, item: ImportSellRemainProductDto) => <div style={{ textAlign: "center" }}>
                    <Image className='imageDetailProductExportExcel' src={(item.fi_id != undefined && item.fi_id.id != undefined) ? this.getImageProduct(item.fi_id.md5 != undefined ? item.fi_id.md5 : "") : AppConsts.appBaseUrl + "/image/no_image.jpg"} style={{ height: `${this.props.is_print ? "70px" : "70px"}`, width: `${this.props.is_print ? "70px" : "70px"}`, maxWidth: '70px !important', maxHeight: '70px !important' }}
            ...
            { title: this.props.title, key: "quantity", sorter:(a,b)=>a.quantity-b.quantity, render: (text: string, item: ImportSellRemainProductDto) => <div>{item.quantity}</div> },
        ];
        return (
            <Modal
                width={'60%'}
                ...
                maskClosable={false}
                title={(<h3>{`${this.props.title} của "`}<b style={{color:"red"}}>{`${this.props.ma_code} - ${this.props.ma_name}`}</b> thuộc nhóm <b style={{color:"red"}}>{`${this.props.groupMachine}`}</b>": <b style={{color:"red"}}>{this.props.listItemRemain.length}</b></h3>)}
                footer={null}
            >
                <Row gutter={16} style={{ marginBottom: 10 }}>
                    <Col {...cssColResponsiveSpan(24, 12, 8, 6, 6, 6)}>
                        <strong>Mã sản phẩm</strong>
                        <Input allowClear placeholder='Mã sản phẩm' value={this.state.codeProduct} onChange={async (e) => {
                            await this.setState({ codeProduct: e.target.value.trim() }); await this.handleSubmitSearch()
                        }} />
                    </Col>
                    <Col {...cssColResponsiveSpan(24, 12, 8, 6, 6, 6)}>
