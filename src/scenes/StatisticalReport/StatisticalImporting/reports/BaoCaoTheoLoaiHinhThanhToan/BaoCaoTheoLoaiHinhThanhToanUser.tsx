...
export default class BaoCaoTheoLoaiHinhThanhToanUser extends AppComponentBase {
    ...
    visibleBarchartReport = () => {
        ...
        if (this.totalFooter.quantity > 0) {
    ...
    render() {
        ...
        const columns = [
            {
                title: "STT", classNames: "start", key: "stt", width: 50, render: (_: string, item: StatisticBillingOfPaymentDto, index: number) => <div>
            ...
            {
                title: "Loại hình thanh toán", key: "name", render: (_: string, item: StatisticBillingOfPaymentDto) => <div>
                    <Tooltip title="Xem chi tiết giao dịch">
                        {item.name === "cash" ? "Tiền mặt" : (item.name === "transaction" ? "Ngân hàng" : "Thẻ RFID")}
                    </Tooltip>
                </div>
            },
            {
                title: "Số đơn hàng đã bán", key: "quantity", sorter: (a, b) => a.quantity - b.quantity, render: (_: string, item: StatisticBillingOfPaymentDto) => <div>
                    <Tooltip title="Xem chi tiết giao dịch">
                        {AppConsts.formatNumber(item.quantity)}
                    </Tooltip>
                </div>
            },
            {
                title: "Tổng doanh thu", key: "total_money",
                sorter: (a, b) => a.total_value - b.total_value,
                render: (_: string, item: StatisticBillingOfPaymentDto) => <div>
                    <Tooltip title="Xem chi tiết giao dịch">
                        {AppConsts.formatNumber(item.total_value)}
                    </Tooltip>
                </div>
            },
