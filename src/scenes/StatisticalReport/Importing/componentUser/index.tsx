...
export default class ImportingUser extends React.Component {
  ...
  async getAll() {
    ...
    await stores.importingStore.getAll(
      this.state.ma_id,
      this.state.im_person_charge,
      this.state.im_start_date,
      this.state.im_end_date,
      this.state.gr_ma_id,
      this.selectedField,
      this.state.sort,
  ...
  onChangePage = async (page: number, pagesize?: number) => {
    ...
    await this.setState(
      { skipCount: (page - 1) * this.state.pageSize, currentPage: page },
      async () => {
        this.getAll();
      }
    );
  ...
  render() {
    ...
    const left = this.state.viewDetail
      ? { ...cssColResponsiveSpan(24, 24, 24, 14, 14, 14) }
      : cssCol(24);
    const right = this.state.viewDetail
      ? { ...cssColResponsiveSpan(24, 24, 24, 10, 10, 10) }
      : cssCol(0);
    return (
      <Card>
        <Row align="bottom" gutter={[8, 8]} className="alighItemFlexEnd-col-1600px">
          <Col {...cssColResponsiveSpan(18, 17, 12, 12, 12, 12)}>
            <h2>Lịch sử nhập sản phẩm</h2>
          </Col>
          {isGranted(AppConsts.Permission.Pages_History_LichSuNhapHang_Export) && (
            <Col {...cssColResponsiveSpan(6, 7, 12, 12, 12, 12)} style={{ textAlign: 'right' }}>
              <Button
                type="primary"
                icon={<ExportOutlined />}
                onClick={() => this.setState({ visibleExportExcelImporting: true })}
              >
                {window.innerWidth > 688 && 'Xuất dữ liệu'}
              </Button>
            </Col>
          )}
        </Row>
        <Row align="bottom" gutter={[8, 8]}>
          ...
          <Col {...cssColResponsiveSpan(24, 12, 8, 8, 8, 4)}>
            ...
            <SelectedGroupMachine
              ...
              onChangeGroupMachine={async (value) => await this.setState({ gr_ma_id: value })}
            />
          </Col>
          <Col {...cssColResponsiveSpan(24, 12, 8, 8, 8, 4)}>
            ...
            <SelectedMachineMultiple
              ...
              listMachineId={this.state.ma_id}
              groupMachineId={this.state.gr_ma_id}
            ></SelectedMachineMultiple>
          </Col>

          <Col
            {...cssColResponsiveSpan(24, 12, 8, 6, 6, 6)}
          ...
          >
            ...
            {(this.state.ma_id != undefined ||
              this.state.im_create_at != undefined ||
