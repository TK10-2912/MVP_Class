...
import { AttachmentItem, FileDto, FileParameter, ImageProductDto } from '@services/services_autogen';
...
export default class FileAttachmentsImagesProduct extends AppComponentBase<IFileAttachmentsProps, IFileAttachmentsStates> {
	...
	uploadImage = async (options) => {
		...
		if (!!fileToUpload.data['size']) {
			let result: ImageProductDto = await stores.imageProductStore.createFile(1, fileToUpload);
