...
export interface IProps {
    onSearchStatistic: (input: SearchWithdrawAdmin) => void;
...
export class SearchWithdrawAdmin {
    ...
    public ma_id_list;
    public payment_type
    constructor(us_id, start_date, end_date, payment_type, ma_id_list) {
        this.us_id = us_id;
...
export default class SearchStaticWithdrawAdmin extends AppComponentBase<IProps> {
    ...
    inputSearch: SearchWithdrawAdmin = new SearchWithdrawAdmin(this.state.us_id, this.state.start_date, this.state.end_date, this.state.payment_type, this.state.listMachineId);
            ...
            <div style={{ width: "100%" }}>
                <Row gutter={[8, 8]} align='bottom'>
                    ...
                    <Col {...cssColResponsiveSpan(24, 12, 8, 5, 5, 4)}>
                        ...
                        <SelectEnum enum_value={this.state.payment_type} onChangeEnum={async (value) => await this.setState({ payment_type: value })} eNum={eReconsile} />
                    </Col>
                    <Col {...cssColResponsiveSpan(24, 12, 8, 5, 5, 3)}>
                        <strong>Máy bán nước</strong>
                        <SelectedMachineMultiple
                            onChangeMachine={(value) => this.setState({ listMachineId: value })} listMachineId={this.state.listMachineId} />
                    </Col>
                    <Col  {...cssColResponsiveSpan(24, 12, 8, 24, 24, 6)} style={{ display: "flex", flexWrap: "wrap", padding: 0 }}>
                        ...
                        {
                            (!!this.state.rangeDatetime  || !!this.state.listMachineId || !!this.state.us_id || !!this.state.payment_type) &&
                            <Col>
