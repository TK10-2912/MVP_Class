...
export interface IProps {
    onSearchStatistic: (input: SearchWithdrawAdmin, type: String) => void;
    getTypeDate?: (type: any) => void;
    datePick?: string;
}
const { RangePicker } = DatePicker;
export class SearchWithdrawAdmin {
    public us_id;
    public start_date;
    public end_date;
    public ma_id_list;
    public payment_type;
    public gr_id;
    constructor(us_id, start_date, end_date, payment_type, ma_id_list, gr_id) {
        this.us_id = us_id;
...
export default class SearchStaticWithdrawAdmin extends AppComponentBase<IProps> {
    state = {
        isLoadDone: false,
    ...
    inputSearch: SearchWithdrawAdmin = new SearchWithdrawAdmin(this.state.us_id, this.state.start_date, this.state.end_date, this.state.payment_type, this.state.listMachineId, this.state.gr_id);
        ...
        return (
            <div style={{ width: "100%" }}>
                <Row gutter={[8, 8]} align='bottom'>
                    <Col {...cssColResponsiveSpan(24, 12, 8, 6, 6, 2)}>
                        <strong>Loại</strong>
                        <Select
                    ...
                    <Col {...cssColResponsiveSpan(24, 12, 8, 6, 6, 4)}>
                        ...
                        <SelectedMachineMultiple
                            groupMachineId={this.state.gr_id}
                            onChangeMachine={(value) => { this.setState({ listMachineId: value }); this.handleSubmitSearch() }} listMachineId={this.state.listMachineId} />
                    </Col>
                    <Col {...cssColResponsiveSpan(24, 12, 8, 6, 6, 3)}>
                        <strong>Người vận hành</strong>
                        <SelectUserMultiple
                            onChangeUser={async (value) => { await this.setState({ us_id: value }); this.handleSubmitSearch() }}
                    ...
                    <Col  {...cssColResponsiveSpan(24, 12, 8, 12, 12, 5)} style={{ display: "flex", flexWrap: "wrap", padding: 0 }}>
                        <Col>
                        ...
                        {
                            (!!this.state.rangeDatetime || !!this.state.listMachineId || !!this.state.us_id || !!this.state.payment_type || !!this.state.gr_id) &&
                            <Col>
